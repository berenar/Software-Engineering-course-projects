...
int tamAI(unsigned int ninodos){
	int AIsize;
	//Utilizaremos el módulo para saber si necesitamos esa cantidad o un bloque adicional
	if((ninodos%(BLOCKSIZE/TAM_INODO))==0){
		AIsize=ninodos/(BLOCKSIZE/TAM_INODO);
	}else {
		AIsize=(ninodos/(BLOCKSIZE/TAM_INODO))+1;
	}
	return AIsize;
}
...
int initAI(unsigned int ninodos){
	...
	struct inodo inodos[BLOCKSIZE/TAM_INODO];
	unsigned int contInodos = SB.posPrimerInodoLibre+1;
	int i;
	for(i=SB.posPrimerBloqueAI; i<=SB.posUltimoBloqueAI;i++){
		int j;
		for(j=0; j<BLOCKSIZE/TAM_INODO; j++){//Hasta que j sea mayor que la cantidad de inodos en el bloque.
...
int escribir_inodo(struct inodo inodo, unsigned int ninodo){
	...
	int nbloc = ((ninodo * TAM_INODO) / BLOCKSIZE)+ SB.posPrimerBloqueAI;
	//Empleamos un array de inodos, del tamaño de la cabenantidad de inodos que caben en 
	//un bloque (BLOCKSIZE/TAM_INODO), como buffer de lectura del bloque que hemos de leer
	struct inodo arrinodos [(BLOCKSIZE / TAM_INODO)]; 											//+ sb.posPrimerBloqueAI;??????????????????????
//memset(arrinodos, 0, BLOCKSIZE);
	...
	if(escritura<0)return -1;
		...
		//del array (ninodo%(BLOCKSIZE/TAM_INODO))
	arrinodos[(ninodo%(BLOCKSIZE/TAM_INODO))]= inodo;
...
int leer_inodo(unsigned int ninodo, struct inodo *inodo){
	...
	//un bloque (BLOCKSIZE/TAM_INODO), como buffer de lectura del bloque que hemos de leer
	struct inodo arrinodos[(BLOCKSIZE/TAM_INODO)];
//memset(arrinodos, 0, BLOCKSIZE);
	int lectura = bread(nbloc,arrinodos);
//memset(arrinodos, 0, BLOCKSIZE);
	//El inodo solicitado está en la posición ninodo%(BLOCKSIZE/TAM_INODO) del buffer
	*inodo = arrinodos[ninodo%(BLOCKSIZE/TAM_INODO)];
	if(lectura<0){
		return -1;
	}else{
		return 0;
